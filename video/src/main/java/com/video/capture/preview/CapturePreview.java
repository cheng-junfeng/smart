...
package com.video.capture.preview;
...
public class CapturePreview implements SurfaceHolder.Callback {
	...
	public void surfaceChanged(final SurfaceHolder holder, final int format, final int width, final int height) {
		...
		try {
			...
			LogUtil.d(TAG, "Configured camera for preview in surface of " + width + " by " + height);
		} catch (final RuntimeException e) {
			e.printStackTrace();
			LogUtil.d(TAG, "Failed to show preview - invalid parameters set to camera preview");
		...
		try {
			...
			e.printStackTrace();
			LogUtil.d(TAG, "AutoFocus not available for preview");
		}
		...
		try {
			...
			e.printStackTrace();
			LogUtil.d(TAG, "Failed to show preview - unable to connect camera to preview (IOException)");
			mInterface.onCapturePreviewFailed();
		} catch (final RuntimeException e) {
			e.printStackTrace();
			LogUtil.d(TAG, "Failed to show preview - unable to start camera preview (RuntimeException)");
			mInterface.onCapturePreviewFailed();
		}
	}
	...
	public void releasePreviewResources() {
		if (mPreviewRunning) {
			try {
				...
				e.printStackTrace();
				LogUtil.e(TAG, "Failed to clean up preview resources");
			}
		}
	}
