...
import com.wu.safe.ahik.utils.CommonMethod;
...
public class JNATest {
	...
	{
		NET_DVR_MULTI_STREAM_COMPRESSIONCFG_COND[] compressCond = (NET_DVR_MULTI_STREAM_COMPRESSIONCFG_COND[])new NET_DVR_MULTI_STREAM_COMPRESSIONCFG_COND().toArray(2);
		NET_DVR_MULTI_STREAM_COMPRESSIONCFG[]	compress = (NET_DVR_MULTI_STREAM_COMPRESSIONCFG[])new NET_DVR_MULTI_STREAM_COMPRESSIONCFG().toArray(2);
		Pointer lpCond = compressCond[0].getPointer();
		Pointer lpParam = compress[0].getPointer();
		
		INT_ARRAY pInt = new INT_ARRAY(2);
	...
	public static class FMSGCallBack implements HCNetSDKByJNA.FMSGCallBack
	{
		...
		public void invoke(int lCommand, NET_DVR_ALARMER pAlarmer,
			...
			{
				NET_DVR_ALARMINFO_V30	struAlarmInfo = new NET_DVR_ALARMINFO_V30(pAlarmInfo);
				struAlarmInfo.read();
				System.out.println("COMM_ALARM_V30 alarm type:" + struAlarmInfo.dwAlarmType);
			}
			else if(lCommand == HCNetSDKByJNA.COMM_ALARM_V40)
			{
				NET_DVR_ALARMINFO_V40	struAlarmInfo = new NET_DVR_ALARMINFO_V40(pAlarmInfo);
				struAlarmInfo.read();
			...
			{
				NET_DVR_PLATE_RESULT	struAlarmInfo = new NET_DVR_PLATE_RESULT(pAlarmInfo);
				struAlarmInfo.read();
			...
			{
				NET_ITS_PLATE_RESULT	struAlarmInfo = new NET_ITS_PLATE_RESULT(pAlarmInfo);
				struAlarmInfo.read();
			...
			{
				NET_VCA_RULE_ALARM	struAlarmInfo = new NET_VCA_RULE_ALARM(pAlarmInfo);
				struAlarmInfo.read();
				if(struAlarmInfo.struRuleInfo.wEventTypeEx == HCNetSDKByJNA.ENUM_VCA_EVENT_EXIT_AREA)
				{
					NET_VCA_AREA	struExit = new NET_VCA_AREA(struAlarmInfo.struRuleInfo.uEventParam.getPointer());
					struExit.read();
				}
	...
	public void Test_Alarm_V41(int iUserID)
	{
		...
		{		
			...
			NET_DVR_SETUPALARM_PARAM 	arlarmParam = new NET_DVR_SETUPALARM_PARAM();
	...
	public void Test_LoiteringDetection(int iUserID, int iChan)
	{
		...
		INT_ARRAY ptrInt = new INT_ARRAY(1);
		ptrInt.iValue[0] = 1;
		ptrInt.write();
		
		NET_DVR_LOITERING_DETECTION struParam = new NET_DVR_LOITERING_DETECTION();
		BYTE_ARRAY ptrByte = new BYTE_ARRAY(1024);
	...
	public static class FRemoteConfigCallBack implements fRemoteConfigCallback
	{
	...
	private static final fRemoteConfigCallback fCallback = new	FRemoteConfigCallBack();
	public void Test_GetInputVolume(int iUserID)
	{
		if(m_lLongConfig == -1)
		{
			NET_DVR_INPUTVOLUME struInputVolume = new NET_DVR_INPUTVOLUME();
	...
	public static class fCardCfgCallBack implements fRemoteConfigCallback
	{
		...
		public void invoke(int dwType, Pointer lpBuffer, int dwBufLen,
			...
			{
				NET_DVR_CARD_CFG struCardCfg = new NET_DVR_CARD_CFG(lpBuffer);
				struCardCfg.read();
	...
	private static final fRemoteConfigCallback fCardCallback = new	fCardCfgCallBack();
	public void Test_GetCardCfg(int iUserID)
	{
		if(m_lCardCfg == -1)
		{
			NET_DVR_CARD_CFG_COND struCardCond = new NET_DVR_CARD_CFG_COND();
	...
	{		
		...
		NET_DVR_LED_AREA_COND struCond = new NET_DVR_LED_AREA_COND();
		...
		NET_DVR_LED_AREA_INFO_LIST struInfoList = new NET_DVR_LED_AREA_INFO_LIST();
		struInfoList.dwSize = struInfoList.size();
		struInfoList.dwLEDAreaNum =1;
		NET_DVR_LED_AREA_INFO [] astruInfo = (NET_DVR_LED_AREA_INFO[])new NET_DVR_LED_AREA_INFO().toArray(128);
	...
	public void TEST_Passive_Decode(int iUserID)
	{
		...
		NET_DVR_MATRIX_PASSIVEMODE struPasMode = new NET_DVR_MATRIX_PASSIVEMODE();
		...
		BYTE_ARRAY ptrByte = new BYTE_ARRAY(30*1024);
		ptrByte.write();
	...
	{
		NET_DVR_ALARMHOST_OTHER_STATUS_V50 struCfg = new NET_DVR_ALARMHOST_OTHER_STATUS_V50();
	...
	{
		NET_DVR_XML_CONFIG_INPUT	inputCfg = new NET_DVR_XML_CONFIG_INPUT();
		inputCfg.dwSize = inputCfg.size();
		String str = "GET /ISAPI/VCS/terminals\r\n";
		BYTE_ARRAY byteArr = new  BYTE_ARRAY(str.length());
		...
		NET_DVR_XML_CONFIG_OUTPUT outputCfg = new NET_DVR_XML_CONFIG_OUTPUT();
		BYTE_ARRAY outBuf = new BYTE_ARRAY(100*1024);
		outBuf.write();
		outputCfg.dwSize = outputCfg.size();
		outputCfg.lpOutBuffer = outBuf.getPointer();
		outputCfg.dwOutBufferSize = 100*1024;
		BYTE_ARRAY statusBuf = new BYTE_ARRAY(1024);
	...
	{
		NET_DVR_OPEN_EZVIZ_USER_LOGIN_INFO loginInfo = new NET_DVR_OPEN_EZVIZ_USER_LOGIN_INFO();
